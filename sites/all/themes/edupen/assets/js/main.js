(function ($) {
    ...
    // bannerHeight();
  const heightMenu = $('.header-section').height();
    $('#offcanvasDashboard').css('margin-top',heightMenu + 20);
    $('.dashboard-main-wrapper').css('padding-top',heightMenu + 20);
    ...
    if($('.danh_muc_catalog').hasClass('tontai'))
    {
        $('.form-de-thi').prepend('<div class="loader"></div>');
        setTimeout(function () {
            $('.loader').remove();
            var html_edit=$('.danh_muc_catalog').closest('div').html();
            $('.danh_muc_catalog').closest('div').remove();
            $('#de-thi-node-form').prepend('<div class="vertical-tabs clearfix">'+html_edit+'</div>');
            var dsselect=$('.form-item-field-danh-muc-und select');
            dsselect.each(function () {
                var ds_option = $(this).find('option');
                var run_need=0;
                var id_trc='';
                ds_option.each(function () {
                    run_need++;
                    var text_need = $(this).text();
                    if(text_need.substr(0,1)!=='-')
                    {
                        if(id_trc!=='')
                        {
                            $('.'+id_trc).prop("disabled", false);
                        }
                        $(this).attr('disabled','disabled');
                        $('.'.id_trc).removeAttr('disabled');
                        $(this).addClass('tat_'+run_need);
                        id_trc='tat_'+run_need;
                    }
                    else
                    {
                        id_trc='';
    ...
    if($('main.dashboard-main-wrapper').length > 0)
    {
        var height_1=$('main.dashboard-main-wrapper').height();
        var height_2=$('.footer-section').height();
        var height_3=screen.availHeight;
        if(height_3>height_1)
        {
            $('main.dashboard-main-wrapper .dashboard-content').css('min-height',(height_3-height_2-230)+'px');
        }
    }
    $(document).on('click','.manipulation .fa-edit', function (e) {
        e.preventDefault();
        // $('#exampleModal_1').modal('show');
        // $('#exampleModal_1 .modal-body').html('<div class="loader"></div>');
        // var id_need=$(this).attr('id_edit');
        // $.ajax({
        //     url: 'https://edupen.vn/form-sua-bai',
        //     data:
        //         {
        //             sua_bai : id_need,
        //         },
        ...
        //         console.log(data);
        //         $('#exampleModal_1 .modal-body').html(data.form_sua);
        //     },
        // })
    })
    ...
    $(document).on('click','.header-category-toggle',function(e){
      e.preventDefault();
      $('.dashboard-page').toggleClass('page-full-width');
      $('#offcanvasDashboard').toggleClass('hide-sidebar');
    })
})(jQuery);
